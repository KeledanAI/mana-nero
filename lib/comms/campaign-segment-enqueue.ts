...
export type StaffCampaignSegment = "newsletter_opt_in" | "marketing_consent";
...
function segmentQuery(supabase: SupabaseClient, segment: StaffCampaignSegment) {
...
export async function enqueueStaffSegmentCampaign(
  ...
  const { data: profiles, error: listErr } = await segmentQuery(supabase, segment);
